    ...
    getmedicine(sku)
      .then(function (response) {
        if (response.data.results && response.data.results.length > 0) {
          ...
          let searchName = sku;
          if (activeMedicine.openfda) {
            if (
              activeMedicine.openfda.brand_name &&
              activeMedicine.openfda.brand_name[0]
            ) {
              searchName = activeMedicine.openfda.brand_name[0];
            } else if (
              activeMedicine.openfda.generic_name &&
              activeMedicine.openfda.generic_name[0]
            ) {
              searchName = activeMedicine.openfda.generic_name[0];
            }
          }
      ...
      <div className="w-full bg-blue-600 text-white shadow-md sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <h1 className="text-lg font-bold tracking-wide truncate max-w-[200px] sm:max-w-none">
            {details.openfda?.brand_name
              ? details.openfda.brand_name[0]
              : 'Medicine Details'}
          </h1>
